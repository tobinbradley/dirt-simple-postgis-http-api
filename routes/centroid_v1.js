...
function formatSQL(request) {
  var transform = `
    ST_Transform(${request.query.force_on_surface
      ? 'ST_PointOnSurface'
      : 'ST_Centroid'}(
      ...
      validate: {
        ...
        query: {
          ...
          srid: Joi.number()
            ...
            .default(4326)
            .description(
              'The SRID for the returned centroid. The default is <em>4326</em> Lat/Lng.'
            ),
          filter: Joi.string().description(
            'Filtering parameters for a SQL WHERE statement.'
          ),
          force_on_surface: Joi.boolean()
            .default(false)
            .description(
              'Set true to force point on surface. The default is the <em>false</em>.'
            )
        }
      },
      jsonp: 'callback',
