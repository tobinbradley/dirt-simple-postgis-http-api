...
function formatSQL(request) {
  ...
  if (request.query.bounds) {
    ...
    if (bounds.length === 4) {
      sql.where(
        `${request.query
          .geom_column} && ST_Transform(ST_MakeEnvelope(${bounds.join(
          ',',
      ...
      validate: {
        ...
        query: {
          ...
          bounds: Joi.string()
            .default('')
            .description(
              'Clipping box for the output. Can be expressed as a bounding box (<em>sw.lng, sw.lat, ne.lng, ne.lat</em>) or a Z/X/Y tile (<em>0,0,0</em>).',
            ),
